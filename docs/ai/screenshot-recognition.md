...
成员可一次上传上一个完整月的 1 至 6 张运动软件截图，由视觉大模型逐图抽取可见运动量并汇总；云函数再按东成西就规则计算等效跑量。模型输出仅是识别建议，成员确认和管理员审核后才可用于结算。
...
同一成员、同一月份只保留一个当前提交文档，文档 ID 为 `activity-用户ID-月份`。当前一批截图保存在 `evidenceFileIds`（最多 6 张），重新上传会增加修订号；上一批的文件 ID 会转入 `previousEvidenceFileIds` 审计保留（最多 12 张）。客户端不直接写数据库。
...
截图识别云函数当前默认模型为 `local-vsr`，已用一张真实截图完成端到端验证。多图时使用一条模型请求携带全部图片，避免串行调用造成超时；函数超时应设置为至少 120 秒。单张图片最大 4 MB，单次所有图片总大小最大 12 MB。
