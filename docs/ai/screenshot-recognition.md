...
模型必须只提取截图中清晰可见、明确标注为累计或当月总量的数据，不得推测缺失数值。每项必须带对应图片序号 `imageIndex`。距离 `rawValue` 保留截图原始数值及原始单位：例如 `48,806 米` 必须输出 `48806` 和 `m`，不能缩写或自行换算。服务端会先将 `m` 转为 `km`，再按运动类型换算等效跑量。无法归类的运动使用 `custom`，系统不自动换算，必须由管理员审核。若多张图片可能展示同一份月度总量，模型不得自行相加，并须要求成员复核。
