...
- `ocr_activity_screenshot`：先建立本批任务，再让每张截图分别调用一次 `local-vsr`，将各图 OCR 原文分片保存后合并到当前活动记录；
...
- `cancel`：成员取消尚未确认的识别结果，状态变为 `cancelled`，立即删除当前记录关联的全部云端截图并重新开放提交通道。
- `withdraw`：成员作废仍处于 `pending_admin_review` 的提交，状态变为 `withdrawn`，立即删除当前记录关联的全部云端截图并重新开放提交通道；已经被管理员处理的记录不可撤回。

同一成员、同一月份只保留一个当前提交文档，文档 ID 为 `activity-用户ID-月份`。当前一批截图保存在 `evidenceFileIds`（最多 3 张），重新上传会增加修订号；未主动作废的上一批文件 ID 可暂存于 `previousEvidenceFileIds`。定时清理函数只保留最近三个提交月份的有效截图；更早截图从云存储删除并清空文件引用，但结构化跑量、识别、审核和结算记录继续保留。客户端不直接写数据库。
...
CloudBase 每个云函数的超时硬上限为 60 秒。客户端先建立 OCR 批次，然后按顺序逐张调用 OCR 函数；每张图均拥有独立的 60 秒窗口，不会因同一请求包含多图而相互挤占时间或被视觉模型漏读。全部 OCR 分片合并后再发起数据判断函数，判断阶段也拥有独立的 60 秒窗口；两个模型的单次内部请求均在 48 秒主动中断，为数据库回写保留余量。如果 OCR 已全部完成后用户关闭页面，再次进入上传页会根据 `ocr_completed` 状态自动继续判断阶段。单批限制为最多 3 张，单张图片最大 4 MB，单次所有图片总大小最大 12 MB。
...
`local-vsr` 禁止判断、筛选、计算或合并数据，只允许原样抄录。`local-premium` 必须先锁定应提交月份，忽略 OCR 中出现的其他月份；每项需返回 `activityMonth`、图片序号和 `sourceLineIndexes`。服务端会重新取出这些 OCR 行，确认月份上下文及 `rawValue` 确实存在；其他月份或无法验证的项目自动丢弃。对目标月份的同一种运动，月度累计或总量有绝对优先级；没有月度汇总时，将整张截图视为一次运动，只取页面明确显示的该次总距离、总次数或累计爬升，不允许把圈数、分段、每公里、按天或多条运动明细自主相加。月度汇总与组成它的明细不得同时计入。服务端再将 `m` 转为 `km`，并按运动类型换算等效跑量。若多张图片可能展示同一份月度总量或同一次运动，判断模型不得自行重复相加，并须要求成员复核。

服务端还会执行单位白名单校验：跑步、骑行、游泳只能为 `km` 或 `m`；跳绳只能为 `count`；累计爬升只能为 `m`。卡路里/大卡/kcal、步数、时长、心率等任何非距离或非跳绳次数都会被丢弃，即使模型错误地将其标成跑步也不能计入等效跑量。

登山、徒步、爬山、Hiking、Trekking 等山地活动按爬升换算：只取截图中明确标注的累计爬升、总爬升、爬升高度或海拔增益米数，不取路线距离，也不把当前海拔、最高海拔或最低海拔误作爬升。没有明确爬升总量时要求成员复核，不用路线公里数替代。
