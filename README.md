...
- 已完成：历史公积金总账对账与展示。旧台账的月度收入合计、8 笔历史支取/调整和隐含期初余额 `+102.00` 元已按来源单元格迁移为可审计流水，合计与 `2026-07` 余额 `-257.00` 元一致。公示页累计缴入和累计支取持续基于确认流水更新；明细仅显示最近一个有流水月份，历史区则按年、每行四个月展示收入、支取和月末余额宫格。用途缺失的旧支取明确标为“旧台账未标注用途”，不编造说明。
