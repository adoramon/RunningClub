...
- 已完成：真实截图智能识别提交。成员可一次上传 1 至 3 张上月截图，由 `local-vsr` 视觉模型逐图识别、合并不同运动记录，再由服务端按跑步、骑行、游泳、跳绳和爬升规则换算；距离会保留截图中的 `km` 或 `m` 原始单位，服务端统一换算后再计算等效跑量。结果会显示每项来自第几张截图，成员确认后进入管理员审核。限制为 3 张是为了适配 CloudBase 云函数 60 秒硬超时。已用真实单图完成一次端到端识别验证（跑步 64.9 km）；多图汇总需在体验版用不同记录截图实际验证。详细配置见 [截图识别接入说明](docs/ai/screenshot-recognition.md)。正式管理员审核、月度结算和公积金流水仍在后续阶段实现。
