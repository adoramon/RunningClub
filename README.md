...
- 已完成：真实截图智能识别提交。上月截图上传至云存储后，由 `local-vsr` 视觉模型识别运动量，再由服务端按跑步、骑行、游泳、跳绳和爬升规则换算；成员确认后进入管理员审核。已用真实截图完成一次端到端识别验证（跑步 64.9 km）；详细配置见 [截图识别接入说明](docs/ai/screenshot-recognition.md)。正式管理员审核、月度结算和公积金流水仍在后续阶段实现。
