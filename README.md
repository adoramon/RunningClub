...
- 开发中：真实截图提交链路。上月截图会上传到云存储，由可配置的视觉大模型识别运动量，再由服务端按跑步、骑行、游泳、跳绳和爬升规则换算。成员确认后进入管理员审核；详细配置见 [截图识别接入说明](docs/ai/screenshot-recognition.md)。在模型环境变量与视觉能力验证完成前，不应作为正式月度申报使用。
